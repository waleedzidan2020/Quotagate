...
def mask_config(c):
    ...
    password_set=bool(x['wifi'].pop('passphrase','')); x['wifi']['password_set']=password_set
    return x
...
def restart_wifi_after_save(c):
    time.sleep(1.0)
    try:
        network.restart_wifi(c); db.event('Wi-Fi settings applied and hostapd restarted','info')
    except Exception as e:
        db.event('Wi-Fi restart failed: '+str(e),'error'); db.alert('wifi-restart-failed','Wi-Fi settings were saved but hostapd failed to restart')
...
def apply(c):
    us=db.users(); ds=db.devices(); rebuilt=network.sync_rules(c,ds,network.blocked(c,us,ds)); network.shaping(c,ds,us); return rebuilt
...
def update_status(c, persist=False):
    ...
    try:
        ...
        remote=(p.stdout.split() or [''])[0]; local=Path('/var/lib/quotagate/installed_commit').read_text().strip() if Path('/var/lib/quotagate/installed_commit').exists() else ''
        result={'ok':True,'current_commit':local,'remote_commit':remote,'available':bool(remote and remote!=local)}
        if persist:
            c['updates']['last_check']=int(time.time()); c['updates']['available_commit']=remote if result['available'] else ''; config.save(c)
        return result
...
class Handler(SimpleHTTPRequestHandler):
    ...
    def hdr(self):
        self.send_header('cache-control','no-store'); self.send_header('pragma','no-cache'); self.send_header('x-frame-options','DENY'); self.send_header('x-content-type-options','nosniff'); self.send_header('referrer-policy','no-referrer')
    ...
    def api_get(self,p,q):
        ...
        if p=='/api/status':
            ...
            return self.json({'users':us,'devices':ds,'events':db.events(80),'alerts':db.alerts(50),'daily':db.daily(),'bundle':{**c['bundle'],'used_bytes':used},'network':c['network'],'wifi':wifi_view(c),'web':c['web'],'mac_rules':db.mac_rules(),'system':system_info(c),'dns':c.get('dns',{})})
        ...
        if p=='/api/2fa/qr':
            ...
            except Exception:return self.json({'uri':uri})
    ...
    def api_post(self,p,d):
        ...
        try:
            ...
            if p=='/api/2fa/setup':
                c['admin']['totp_secret']=auth.new_totp_secret(); c['admin']['totp_enabled']=False; config.save(c); return self.json({'ok':True,'uri':auth.otpauth_uri(c['admin']['totp_secret'])})
            if p=='/api/2fa/enable':
