...
_RULE_SIG=None; _SHAPE_SIG=None; _BLOCK_SIG=None; _DHCP_SIG=None; _OUI_CACHE=None
...
def _load_oui():
    global _OUI_CACHE
    if _OUI_CACHE is not None:return _OUI_CACHE
    cache={}
    for path in ('/usr/share/ieee-data/oui.txt','/usr/share/hwdata/oui.txt'):
        p=Path(path)
        if not p.exists():continue
        try:
            for line in p.read_text(errors='ignore').splitlines():
                m=re.match(r'^([0-9A-Fa-f]{2})[-:]?([0-9A-Fa-f]{2})[-:]?([0-9A-Fa-f]{2})\s+\((?:hex|base 16)\)\s+(.+)$',line.strip())
                if m:cache[''.join(m.group(i).upper() for i in (1,2,3))]=m.group(4).strip()[:80]
        except Exception:pass
        if cache:break
    cache.update({k:v for k,v in {'001302':'Intel','0016CE':'Dell','0019D2':'Intel','001E4F':'Dell','00216A':'Intel','001A92':'ASUSTek','F0DCE2':'Apple','3C5A37':'Samsung','A4C361':'Apple','B827EB':'Raspberry Pi'}.items() if k not in cache})
    _OUI_CACHE=cache;return cache

def manufacturer(mac):return _load_oui().get(mac.upper().replace(':','')[:6],'')

def leases():
    out=[]
    for path in ('/run/quotagate/dnsmasq.leases','/var/lib/misc/dnsmasq.leases','/var/lib/quotagate/dnsmasq.leases'):
        p=Path(path)
        if not p.exists():continue
        try:
            for line in p.read_text(errors='ignore').splitlines():
                x=line.split()
                if len(x)>=4 and re.fullmatch(r'[0-9a-fA-F:]{17}',x[1]):out.append({'mac':x[1].lower(),'ip':x[2],'name':'' if x[3]=='*' else x[3]})
        except Exception:pass
    uniq={}
...
def neighbors(iface):
    out=[];txt=run(['ip','neigh','show','dev',iface]).stdout
...
def discover(iface):
    ...
    for mac,ip in neighbors(iface):by_mac.setdefault(mac,{'mac':mac,'ip':ip,'name':''})['ip']=ip
...
def ensure_lan(c):
    ...
    run(['ip','link','set',lan,'up'],True);run(['ip','addr','replace',f"{n['lan_ip']}/{n['lan_prefix']}",'dev',lan],True)
...
def write_hostapd(c):
    ...
    runtime=Path('/run/quotagate');runtime.mkdir(parents=True,exist_ok=True,mode=0o700);target=runtime/'hostapd.conf';target.write_text(text);target.chmod(0o600)

def _known_dhcp_macs(c):
    rules={r['mac'].lower():r['action'] for r in db.mac_rules()};out=set()
    for d in db.devices():
        mac=str(d.get('mac','')).lower()
        if rules.get(mac)=='blacklist':continue
        if rules.get(mac)=='whitelist' or (d.get('enabled') and not d.get('blocked_manual')):out.add(mac)
    out.update(mac for mac,a in rules.items() if a=='whitelist')
    return sorted(x for x in out if re.fullmatch(r'[0-9a-f:]{17}',x))

def _dhcp_signature(c):
    n=c['network'];return (n.get('pool_start'),n.get('pool_end'),n.get('lan_ip'),n.get('stop_new_connections'),tuple(_known_dhcp_macs(c)) if n.get('stop_new_connections') else ())

def write_dnsmasq(c):
    global _DHCP_SIG
    n=c['network'];lines=['# QuotaGate dedicated DHCP only','port=0',f"interface={n['lan_interface']}",'bind-interfaces',f"dhcp-range={n['pool_start']},{n['pool_end']},255.255.255.0,12h",f"dhcp-option=3,{n['lan_ip']}",f"dhcp-option=6,{n['lan_ip']}",'dhcp-authoritative','dhcp-leasefile=/run/quotagate/dnsmasq.leases','log-dhcp']
    if n.get('stop_new_connections'):
        for mac in _known_dhcp_macs(c):lines.append(f'dhcp-host={mac},set:qgknown')
        lines.append('dhcp-ignore=tag:!qgknown')
    runtime=Path('/run/quotagate');runtime.mkdir(parents=True,exist_ok=True,mode=0o700);target=runtime/'dnsmasq.conf';target.write_text('\n'.join(lines)+'\n');target.chmod(0o600);_DHCP_SIG=_dhcp_signature(c)

def start_network_daemons(c):
    run(['pkill','-f','hostapd.*quotagate|hostapd.*hostapd.conf']);run(['pkill','-F','/run/quotagate-dnsmasq.pid']);write_hostapd(c);write_dnsmasq(c)
...
def restart_wifi(c):
    ensure_lan(c);write_hostapd(c);run(['pkill','-F','/run/quotagate-hostapd.pid']);run(['pkill','-f','hostapd.*quotagate|hostapd.*hostapd.conf'])
...
def restart_dnsmasq(c):
    write_dnsmasq(c);run(['pkill','-F','/run/quotagate-dnsmasq.pid']);p=run(['dnsmasq','--conf-file=/run/quotagate/dnsmasq.conf','--pid-file=/run/quotagate-dnsmasq.pid'])
    if p.returncode:raise RuntimeError((p.stderr or p.stdout or 'dnsmasq failed to restart').strip())

def sync_dnsmasq(c):
    global _DHCP_SIG
    sig=_dhcp_signature(c)
    if _DHCP_SIG==sig:return False
    if Path('/run/quotagate-dnsmasq.pid').exists():restart_dnsmasq(c)
    else:write_dnsmasq(c)
    _DHCP_SIG=sig;return True

def stop_network_daemons():run(['pkill','-F','/run/quotagate-hostapd.pid']);run(['pkill','-F','/run/quotagate-dnsmasq.pid'])

def detect_vpn_interface():
    p=run(['ip','-o','link','show']);names=[]
...
def rebuild_rules(c,devices):
    ...
    run(['nft','flush','chain','inet','quotagate','forward']);run(['nft','flush','chain','inet','quotagate','input']);run(['nft','flush','chain','inet','quotagate','output'])
    run(['nft','add','rule','inet','quotagate','input','iifname',wan,'counter','comment','qg:gateway:down']);run(['nft','add','rule','inet','quotagate','output','oifname',wan,'counter','comment','qg:gateway:up'])
    for d in devices:
        ip=d.get('ip','')
...
def gateway_counters():
    p=run(['nft','-j','list','table','inet','quotagate'])
    if p.returncode:return {'up':0,'down':0}
    try:j=json.loads(p.stdout)
    except Exception:return {'up':0,'down':0}
    out={'up':0,'down':0}
    for x in j.get('nftables',[]):
        r=x.get('rule')
        if not r:continue
        m=re.fullmatch(r'qg:gateway:(up|down)',r.get('comment',''))
        if not m:continue
        for e in r.get('expr',[]):
            if 'counter' in e:out[m.group(1)]=int(e['counter'].get('bytes',0))
    return out
...
def effective_quota(user,users,c):
    ...
    if mode=='auto':
        fixed=sum(float(x.get('quota_gb') or 0)+float(x.get('topup_gb') or 0) for x in users if str(x.get('quota_mode','fixed')).lower() not in ('shared','auto','disabled') and x.get('enabled'));auto=[x for x in users if str(x.get('quota_mode','')).lower() in ('shared','auto') and x.get('enabled')];base=max(0.0,float(c['bundle']['total_gb'])-fixed)/max(1,len(auto));return base+float(user.get('topup_gb') or 0)
...
def blocked(c,users,devices):
    ...
    for d in devices:
        ip=d.get('ip','');mac=d.get('mac','').lower();u=um.get(d.get('user_id'));action=rules.get(mac);b=bool(d.get('blocked_manual')) or not bool(d.get('enabled'))
        if u:
...
def _shape_one(iface,direction,devices,users,c):
    run(['tc','qdisc','del','dev',iface,'root']);um={u['id']:u for u in users};key='speed_down_kbit' if direction=='down' else 'speed_up_kbit';limited=[]
    for d in devices:
    ...
    run(['tc','class','add','dev',iface,'parent','1:','classid','1:999','htb','rate',f'{total}kbit','ceil',f'{total}kbit']);run(['tc','qdisc','add','dev',iface,'parent','1:999','fq_codel']);cls=10
    for ip,rate in limited:
        rate=max(8,min(rate,total));cid=f'1:{cls}';run(['tc','class','add','dev',iface,'parent','1:','classid',cid,'htb','rate',f'{rate}kbit','ceil',f'{rate}kbit']);field='dst' if direction=='down' else 'src';run(['tc','filter','add','dev',iface,'protocol','ip','parent','1:','prio','1','u32','match','ip',field,ip,'flowid',cid]);run(['tc','qdisc','add','dev',iface,'parent',cid,'fq_codel']);cls+=1

def shaping(c,devices,users):
    global _SHAPE_SIG
    if not c.get('features',{}).get('speed_limits',True):clear_shaping(c);return True
    n=c['network'];w=wan_interface(c);sig=(tuple((d['id'],d.get('ip'),d.get('speed_down_kbit'),d.get('speed_up_kbit')) for d in devices),tuple((u['id'],u.get('speed_down_kbit'),u.get('speed_up_kbit')) for u in users),n.get('line_down_mbit'),n.get('line_up_mbit'),w)
...
def runtime(c):ensure_lan(c);write_hostapd(c);write_dnsmasq(c);ensure_firewall(c)
