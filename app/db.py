...
CREATE TABLE IF NOT EXISTS gateway_usage_monthly(
...
def con():
    DB.parent.mkdir(parents=True, exist_ok=True, mode=0o700); os.chmod(DB.parent, 0o700)
...
def event(message,level='info'):
    with L,con() as c:c.execute('INSERT INTO events(ts,level,message) VALUES(?,?,?)',(int(time.time()),level,str(message)[:2000]))
def alert(kind,message):
    with L,con() as c:c.execute('INSERT INTO alerts(ts,kind,message) VALUES(?,?,?)',(int(time.time()),str(kind)[:50],str(message)[:1000]))
def events(n=200):
...
def users():
    q='''SELECT u.*,COALESCE(SUM(m.up_bytes+m.down_bytes),0) usage_bytes,COUNT(DISTINCT d.id) device_count
...
def devices():
    q='''SELECT d.*,u.name user_name,COALESCE(m.up_bytes,0) up_bytes,COALESCE(m.down_bytes,0) down_bytes
...
def device_by_ip(ip):
    with con() as c:
        r=c.execute('SELECT * FROM devices WHERE ip=? ORDER BY last_seen DESC LIMIT 1',(ip,)).fetchone();return dict(r) if r else None
...
def upsert_device(mac,ip,name=None,manufacturer=''):
    ...
    with L,con() as c:
        ...
        if r:
            ...
            if new_name and (not r['name'] or r['name']==mac):c.execute('UPDATE devices SET ip=?,last_seen=?,name=?,manufacturer=CASE WHEN manufacturer="" THEN ? ELSE manufacturer END WHERE id=?',(ip,now,new_name,manufacturer,r['id']))
            else:c.execute('UPDATE devices SET ip=?,last_seen=?,manufacturer=CASE WHEN manufacturer="" THEN ? ELSE manufacturer END WHERE id=?',(ip,now,manufacturer,r['id']))
            return r['id'],False
        i=c.execute('INSERT INTO devices(name,mac,ip,manufacturer,last_seen) VALUES(?,?,?,?,?)',(name or mac,mac,ip,manufacturer,now)).lastrowid;return i,True

def _usage_upsert(c,table,key,ident,up,down):
    q=f'''INSERT INTO {table}(device_id,{key},up_bytes,down_bytes) VALUES(?,?,?,?)
          ON CONFLICT(device_id,{key}) DO UPDATE SET up_bytes=up_bytes+excluded.up_bytes,down_bytes=down_bytes+excluded.down_bytes'''
    c.execute(q,(int(ident),period() if key=='period' else day(),int(up),int(down)))
def add_usage(i,up,down):
    add_usage_batch([(i,up,down)])
def add_usage_batch(rows):
    clean=[(int(i),max(0,int(up)),max(0,int(down))) for i,up,down in rows if int(up)>0 or int(down)>0]
    if not clean:return
    with L,con() as c:
        for i,up,down in clean:
            _usage_upsert(c,'usage_monthly','period',i,up,down);_usage_upsert(c,'usage_daily','day',i,up,down)

def add_gateway_usage(up,down):
    up=max(0,int(up));down=max(0,int(down))
    if not up and not down:return
    with L,con() as c:
        c.execute('''INSERT INTO gateway_usage_monthly(period,up_bytes,down_bytes) VALUES(?,?,?)
                     ON CONFLICT(period) DO UPDATE SET up_bytes=up_bytes+excluded.up_bytes,down_bytes=down_bytes+excluded.down_bytes''',(period(),up,down))
        c.execute('''INSERT INTO gateway_usage_daily(day,up_bytes,down_bytes) VALUES(?,?,?)
                     ON CONFLICT(day) DO UPDATE SET up_bytes=up_bytes+excluded.up_bytes,down_bytes=down_bytes+excluded.down_bytes''',(day(),up,down))
def gateway_usage():
    with con() as c:
        r=c.execute('SELECT up_bytes,down_bytes FROM gateway_usage_monthly WHERE period=?',(period(),)).fetchone();return dict(r) if r else {'up_bytes':0,'down_bytes':0}

def create_user(name,quota_gb=0,down=0,up=0,quota_mode='fixed'):
    mode=str(quota_mode).lower();mode='auto' if mode=='shared' else mode
    if mode not in ('fixed','auto','disabled'):raise ValueError('quota_mode must be fixed, auto or disabled')
    with L,con() as c:return c.execute('''INSERT INTO users(name,quota_mode,quota_gb,speed_down_kbit,speed_up_kbit,created_at) VALUES(?,?,?,?,?,?)''',(name,mode,float(quota_gb),int(down),int(up),int(time.time()))).lastrowid
...
def update_device(i,**kw):_update('devices',i,kw,{'name','user_id','exempt','blocked_manual','enabled','is_guest','speed_down_kbit','speed_up_kbit','dns_server','ip','manufacturer'})
def update_user(i,**kw):_update('users',i,kw,{'name','quota_mode','quota_gb','topup_gb','speed_down_kbit','speed_up_kbit','exempt','enabled','history_days','dns_server'})
def delete_user(i):
...
def reset_month():
    with L,con() as c:
        c.execute('DELETE FROM usage_monthly WHERE period=?',(period(),));c.execute('DELETE FROM gateway_usage_monthly WHERE period=?',(period(),));c.execute('UPDATE users SET topup_gb=0')
    event('Monthly counters reset','warning')
def daily(n=31):
    with con() as c:
        return [dict(r) for r in c.execute('''SELECT day,SUM(bytes) bytes FROM (
            SELECT day,SUM(up_bytes+down_bytes) bytes FROM usage_daily GROUP BY day
            UNION ALL SELECT day,(up_bytes+down_bytes) bytes FROM gateway_usage_daily
        ) GROUP BY day ORDER BY day DESC LIMIT ?''',(int(n),))]
...
def mac_rule(mac,action=None,note=''):
    ...
    with L,con() as c:
        ...
        c.execute('''INSERT INTO mac_rules(mac,action,note,created_at) VALUES(?,?,?,?) ON CONFLICT(mac) DO UPDATE SET action=excluded.action,note=excluded.note''',(mac,action,note,int(time.time())))
def mac_rules():
...
def log_dns(client_ip,domain,qtype='A',action='allow'):
    ...
    with L,con() as c:c.execute('INSERT INTO dns_history(ts,device_id,client_ip,domain,qtype,action) VALUES(?,?,?,?,?,?)',(int(time.time()),did,client_ip,domain[:253],qtype[:10],action[:20]))
def prune_dns(default_days=7,max_rows=100000):
    ...
    with L,con() as c:
        c.execute('DELETE FROM dns_history WHERE ts<? AND (device_id IS NULL OR device_id NOT IN (SELECT d.id FROM devices d JOIN users u ON u.id=d.user_id WHERE u.history_days>?))',(now-days*86400,days))
        for u in c.execute('SELECT id,history_days FROM users WHERE history_days!=?',(days,)).fetchall():
            ud=max(1,min(int(u['history_days']),90));c.execute('DELETE FROM dns_history WHERE ts<? AND device_id IN (SELECT id FROM devices WHERE user_id=?)',(now-ud*86400,int(u['id'])))
        r=c.execute('SELECT COUNT(*) n FROM dns_history').fetchone();extra=max(0,int(r['n'])-max_rows)
...
def dns_history(device_id=0,hours=24,limit=500):
    ...
    with con() as c:
        if int(device_id or 0):rows=c.execute('''SELECT h.*,d.name device_name,d.user_id,u.name user_name FROM dns_history h LEFT JOIN devices d ON d.id=h.device_id LEFT JOIN users u ON u.id=d.user_id WHERE h.device_id=? AND h.ts>=? ORDER BY h.id DESC LIMIT ?''',(int(device_id),since,limit))
        else:rows=c.execute('''SELECT h.*,d.name device_name,d.user_id,u.name user_name FROM dns_history h LEFT JOIN devices d ON d.id=h.device_id LEFT JOIN users u ON u.id=d.user_id WHERE h.ts>=? ORDER BY h.id DESC LIMIT ?''',(since,limit))
...
def dns_top(device_id=0,hours=24,limit=30):
    ...
    with con() as c:return [dict(r) for r in c.execute(f'SELECT domain,COUNT(*) hits FROM dns_history WHERE {where} GROUP BY domain ORDER BY hits DESC LIMIT ?',args)]

def firewall_rules():
    with con() as c:return [dict(r) for r in c.execute('SELECT * FROM firewall_rules ORDER BY priority,id')]
def add_firewall_rule(d):
    with L,con() as c:return c.execute('''INSERT INTO firewall_rules(name,direction,src,dst,proto,sport,dport,action,enabled,priority) VALUES(?,?,?,?,?,?,?,?,?,?)''',(d.get('name','rule'),d.get('direction','forward'),d.get('src',''),d.get('dst',''),d.get('proto','any'),d.get('sport',''),d.get('dport',''),d.get('action','accept'),1 if d.get('enabled',True) else 0,int(d.get('priority',100)))).lastrowid
def update_firewall_rule(i,d):_update('firewall_rules',i,d,{'name','direction','src','dst','proto','sport','dport','action','enabled','priority'})
...
def add_port_forward(d):
    with L,con() as c:return c.execute('INSERT INTO port_forwards(name,proto,external_port,internal_ip,internal_port,enabled) VALUES(?,?,?,?,?,?)',(d.get('name','forward'),d.get('proto','tcp'),int(d['external_port']),d['internal_ip'],int(d.get('internal_port',d['external_port'])),1 if d.get('enabled',True) else 0)).lastrowid
def update_port_forward(i,d):_update('port_forwards',i,d,{'name','proto','external_port','internal_ip','internal_port','enabled'})
...
def set_setting(key,value):
    with L,con() as c:c.execute('INSERT INTO settings(key,value) VALUES(?,?) ON CONFLICT(key) DO UPDATE SET value=excluded.value',(key,json.dumps(value)))
